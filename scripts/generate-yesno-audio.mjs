#!/usr/bin/env node
// Generate ElevenLabs audio for every line in yesno/audio/lines.json.
// Dedicated copy of scripts/generate-audio.mjs pointed at the yes/no game so
// the Ballycrann pipeline stays untouched. Skips lines whose MP3 already
// exists, so re-running won't burn credits.
//
// Usage:
//   ELEVENLABS_API_KEY=... node scripts/generate-yesno-audio.mjs
//   node scripts/generate-yesno-audio.mjs --dry-run     (no API calls, just plan)
//   node scripts/generate-yesno-audio.mjs --force       (re-generate even if file exists)
//   node scripts/generate-yesno-audio.mjs --only=yn-q   (only IDs starting with prefix)
...
const AUDIO_DIR = resolve(ROOT, 'yesno', 'audio');
const LINES_PATH = resolve(AUDIO_DIR, 'lines.json');

// Same voice + model as Ballycrann so the two games share Mary's voice.
